...
def load_dataset(data_path, loading_mode):
    ...
    if loading_mode == "split":
        ...
        for filepath in files_split:
            try:
                tensors.append(json.load(filepath)['docs'])
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if torch.Tensor(-1) in tokens:
            # everything from tokens.index(torch.Tensor(-1)) to end becomes 0
            # repeat initializing start_idx with random offset again
            return 
